...
from .amounts import parse_amount_to_call_from_action_text
...
def validate_action_inputs(
    *,
    ...
    labels = _apply_bet_raise_rules(labels, to_call)
...
def _read_amount_to_call(call_button_raw: str | None) -> AmountToCallRead:
    ...
    return AmountToCallRead(call_button_raw.strip(), parsed, status)
...
def _apply_bet_raise_rules(labels: tuple[str, ...], to_call: float | None) -> tuple[str, ...]:
    label_set = set(labels)
    facing_bet = to_call is not None and to_call > 0

    if "bet" in label_set and "raise" in label_set:
        if facing_bet:
            label_set.discard("bet")
        else:
            label_set.discard("raise")
    elif "bet" in label_set or "raise" in label_set:
        if not facing_bet and "raise" in label_set and "bet" not in label_set:
            label_set.remove("raise")
            label_set.add("bet")
        elif facing_bet and "bet" in label_set and "raise" not in label_set:
            label_set.remove("bet")
            label_set.add("raise")
